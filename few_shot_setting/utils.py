        ...
        for line in f:
            ...
            if line == '[Term]':
                ...
                if l[0] == 'id':
                    ...
                    obj['def'].append(line.split('def: ')[1].split('"')[1])
                elif l[0] == 'is_a':
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            self.trace_func(f'Evaluation updated ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save(model, self.path)
        self.val_loss_min = val_loss


def get_logger(dataset, fsl_limit):
    # 第一步，创建一个logger
    ...
    log_name = log_path + dataset + 'fsl_{}'.format(fsl_limit) + '.log'
    logfile = log_name
